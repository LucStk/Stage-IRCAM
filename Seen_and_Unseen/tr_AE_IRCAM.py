...
longoptions = ['lock=', 'load=', 'load_SER=', 'name=']
...
with tf.device(comp_device) :
    ...
    log_dir        = "logs/AE_logs/"+ov.get("--name") + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
    summary_writer = tf.summary.create_file_writer(log_dir)
    
    #Préparation enregistrement
    audio_log_dir        = "logs/audio_logs/AE"+ov.get("--name") + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
